...
| 3 | **Android suspends a WebView's JS timers when its activity stops**, which halts the mining loop mid-session. It is *not* an unfixable Chromium rule: `WebView.resumeTimers()` is process-wide and undoes it. | **Background / screen-off mining works**, on three legs together: (a) a foreground service holding a `PARTIAL_WAKE_LOCK`, (b) the battery-optimisation exemption, or Doze freezes the process regardless, and (c) a ~10s keepalive re-asserting `resumeTimers()` (see `_timerKeepaliveTimer` in `webview_bee_miner.dart`). Drop any one and it stalls. This corrects an earlier claim here that background mining was impossible — the other repo's miner has done it since `51f1b8d`/`ee13de2`, by removing its own `webView.onPause()` call and adding the same keepalive. |
